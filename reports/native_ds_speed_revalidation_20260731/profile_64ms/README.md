...
- Raw range events: 10468
- Sequence delivery: 94.03%
...
- Reference geometry: Fresh static profile revalidation on unchanged setup; cached geometry generation 72 retained only for comparable precision reconstruction. Native DS-TWR 64 ms speed-study timing 15/4/8/2/2 ms, no filtering.
...
|---|---:|---:|---:|---:|---:|---:|
| Coherent independent frames | 10.92 | 0.99 | 0.96 | 1.12 | 2.41 | 3.98 |
| Rolling latest ranges | 58.14 | 1.00 | 0.95 | 1.14 | 2.39 | 5.04 |
...
- `data/profile_64ms_speed_reference_180s.jsonl` — deduplicated DS-TWR range events and status snapshots
- `data/profile_64ms_speed_reference_180s.logs.jsonl` — raw dashboard log records, including every `UWB_RANGING result` line
- `data/profile_64ms_speed_reference_180s.metadata.json` — full initial/final module status and capture settings
