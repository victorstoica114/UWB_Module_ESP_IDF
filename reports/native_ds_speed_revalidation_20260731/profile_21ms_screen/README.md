...
- Raw range events: 9483
- Sequence delivery: 87.90%
- Radio: channel 5 · slot 6 ms · round gap 1 ms · RX slice 100 ms
...
- Reference geometry: 60 s static screening, unchanged setup. Native DS-TWR 21 ms timing 5/1/5/2/2 ms, no filtering.
...
|---|---:|---:|---:|---:|---:|---:|
| Coherent independent frames | 33.34 | 1.06 | 0.96 | 1.18 | 2.53 | 4.41 |
| Rolling latest ranges | 157.85 | 1.04 | 0.95 | 1.17 | 2.45 | 4.54 |
...
- `data/profile_21ms_screen_60s.jsonl` — deduplicated DS-TWR range events and status snapshots
- `data/profile_21ms_screen_60s.logs.jsonl` — raw dashboard log records, including every `UWB_RANGING result` line
- `data/profile_21ms_screen_60s.metadata.json` — full initial/final module status and capture settings
