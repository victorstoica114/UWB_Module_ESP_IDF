...
- Raw range events: 4462
- Sequence delivery: 92.46%
- Radio: channel 5 · slot 7 ms · round gap 1 ms · RX slice 100 ms
...
- Reference geometry: 60 s binary-search screening, unchanged setup. Native DS-TWR candidate 49 ms timing 12/1/8/2/2 ms, no filtering.
...
|---|---:|---:|---:|---:|---:|---:|
| Coherent independent frames | 17.61 | 0.94 | 0.90 | 1.10 | 2.25 | 4.07 |
| Rolling latest ranges | 74.26 | 0.94 | 0.90 | 1.08 | 2.25 | 4.21 |
...
- `data/candidate_49ms_screen_60s.jsonl` — deduplicated DS-TWR range events and status snapshots
- `data/candidate_49ms_screen_60s.logs.jsonl` — raw dashboard log records, including every `UWB_RANGING result` line
- `data/candidate_49ms_screen_60s.metadata.json` — full initial/final module status and capture settings
