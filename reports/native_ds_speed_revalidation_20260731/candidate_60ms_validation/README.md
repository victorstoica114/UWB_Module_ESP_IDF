...
- Raw range events: 11158
- Sequence delivery: 93.97%
...
- Reference geometry: Unchanged setup; decisive validation of 14 ms slot plus safe 4 ms gap (60 ms frame).
...
|---|---:|---:|---:|---:|---:|---:|
| Coherent independent frames | 11.63 | 1.00 | 0.95 | 1.12 | 2.39 | 4.17 |
| Rolling latest ranges | 61.96 | 0.98 | 0.96 | 1.13 | 2.39 | 5.52 |
...
- `data/candidate_60ms_validation_180s.jsonl` — deduplicated DS-TWR range events and status snapshots
- `data/candidate_60ms_validation_180s.logs.jsonl` — raw dashboard log records, including every `UWB_RANGING result` line
- `data/candidate_60ms_validation_180s.metadata.json` — full initial/final module status and capture settings
