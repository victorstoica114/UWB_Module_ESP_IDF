...
- Reference geometry: Calibrated 3.000 m square; static centered tag; 60 ms profile; one anchor survey every four tag frames; 4 ms survey timeout
...
|---|---:|---:|---:|---:|---:|---:|
| Coherent independent frames | 16.96 | 1.16 | 1.19 | 1.37 | 2.90 | 5.18 |
| Rolling latest ranges | 75.66 | 1.18 | 1.18 | 1.38 | 2.92 | 6.21 |
...
- `data/calibrated_60ms_survey_decimated_180s.jsonl` — deduplicated DS-TWR range events and status snapshots
- `data/calibrated_60ms_survey_decimated_180s.logs.jsonl` — raw dashboard log records, including every `UWB_RANGING result` line
- `data/calibrated_60ms_survey_decimated_180s.metadata.json` — full initial/final module status and capture settings
