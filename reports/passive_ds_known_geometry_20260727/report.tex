...
The final static position therefore improves. A direct before/after
comparison also shows that the range calibration works very well, while the
directed-observation calibration still interacts with the range correction
and requires a joint refit.
\end{abstract}
...
\begin{tabular}{@{}lrrl@{}}
\toprule
Calibration effect & Before [cm] & After [cm] & Change \\
\midrule
Live position RMSE & 4.12 & 3.33 & \(19.2\%\) better \\
Position-bias magnitude & 3.51 & 0.96 & \(72.7\%\) better \\
Mean absolute effective-range bias & 5.63 & 0.17 & \(96.9\%\) better \\
Mean absolute directed-path bias & 4.65 & 5.33 & \(14.7\%\) worse \\
\bottomrule
\end{tabular}
\end{center}

The current calibration therefore helps the final position and nearly removes
pair-range bias, but it does not yet independently calibrate every directed
observation.
...
This uses only \(N-1\) independent values for \(N\) anchors and fits the
antisymmetric component of all 12 directed paths. The piggybacked DS ranges
use one additional correction per unordered anchor pair:
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/directed_observation_bias.pdf}
\caption{Direct before/after comparison on the same scale. The current
combined calibration does not move every directed-path median toward zero.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.82\textwidth]{figures/anchor_pair_range_bias.pdf}
\caption{The effective anchor range used by the tag before and after
calibration. Pair-range bias is almost completely removed.}
\end{figure}
...
displaced by several centimeters. The direct comparison supports the
following conclusions:

\begin{itemize}
  \item CFO correction of the responder reply interval is working;
  \item piggybacked DS ranges clearly benefit from per-pair corrections;
  \item final static position RMSE and bias improve;
  \item the independent per-anchor observation fit is incomplete once the
        range correction is active;
  \item autonomous geometry and the observation/range interaction remain
        accuracy penalties.
\end{itemize}

The next implementation step is a joint observation fit performed after range
correction, with an antisymmetric per-anchor term and, if needed, a symmetric
per-pair observation term. Pair ranges should then be robustly filtered before
they update geometry: a rolling median or Huber estimate, a physical-change
gate, and a slower geometry-update cadence. Validation should cover several
