...
- Raw range events: 8242
- Sequence delivery: 89.28%
...
- Reference geometry: 60 s static screening, unchanged setup. Native DS-TWR 25 ms timing 6/1/5/2/2 ms, no filtering.
...
|---|---:|---:|---:|---:|---:|---:|
| Coherent independent frames | 27.09 | 1.04 | 0.98 | 1.21 | 2.47 | 4.20 |
| Rolling latest ranges | 137.28 | 1.05 | 0.99 | 1.19 | 2.50 | 4.19 |
...
- `data/profile_25ms_screen_60s.jsonl` — deduplicated DS-TWR range events and status snapshots
- `data/profile_25ms_screen_60s.logs.jsonl` — raw dashboard log records, including every `UWB_RANGING result` line
- `data/profile_25ms_screen_60s.metadata.json` — full initial/final module status and capture settings
