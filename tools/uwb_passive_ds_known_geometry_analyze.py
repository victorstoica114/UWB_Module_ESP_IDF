...
def main() -> int:
    ...
    for initiator in ANCHOR_IDS:
        for responder in ANCHOR_IDS:
            ...
            pair_values = [
                ...
                * 100.0
                for item in captures["Uncalibrated"]["observations"]
                if int(item["initiator_id"]) == initiator
                and int(item["responder_id"]) == responder
            ]
            labels.append(f"{initiator}→{responder}")
    ...
    axis.set_ylabel("uncalibrated observation error [cm]")
    axis.set_title("Directed observation bias before calibration")
    save_figure(figure, "directed_observation_bias")

    figure, axis = plt.subplots(figsize=(7.5, 4.2))
    labels = [
        f"{row['anchor_a_id']}-{row['anchor_b_id']}"
        for row in baseline_ranges
    ]
    means = [float(row["mean_error_cm"]) for row in baseline_ranges]
    standard_deviations = [float(row["std_cm"]) for row in baseline_ranges]
    axis.bar(labels, means, yerr=standard_deviations, capsize=3, color="#64748b")
    axis.axhline(0, color="black", linewidth=0.9)
    axis.set_xlabel("anchor pair")
    axis.set_ylabel("DS range error [cm]")
    axis.set_title("Anchor-pair DS-TWR bias (mean ± 1σ)")
